...
type FormProps = API.UpdatePasswordParams & { confirmPassword: string };

type FieldProps = {
    field: InputProps,
    form: FormikProps<API.UpdatePasswordParams & { confirmPassword: string }>,
    meta: FieldMetaProps<API.UpdatePasswordParams & { confirmPassword: string }>
}
    ...
    const handleSubmit = async (values: FormProps) => {
        try {
            ...
            if (result.status === 200) {
                Toast({
                    title: t('user.updatePasswordSuccess'),
                    status: 'success',
                    duration: 3000,
                    isClosable: false,
                });
